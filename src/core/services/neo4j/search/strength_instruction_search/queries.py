def strength_instruction_search_query(drug_clause, strength_clause, route_clause, form_clause) -> str:
    return f"""
    MATCH (d:Drug)
    ...
    LIMIT $limit
    """
def strength_focused_search_query(drug_clause, strength_clause) -> str:
    return f"""
    MATCH (d:Drug)
    WHERE ({drug_clause})
        AND ({strength_clause})
