...
search_brand_exact_query="""
                ...
                WHERE d.term_type IN ['SBD', 'BPCK', 'SCD', 'GPCK']
                  AND (toLower(d.name) = toLower($drug_name) 
                       ...
                       OR toLower(d.brand_name) = toLower($drug_name))
...
search_brand_fuzzy_query="""
                ...
                WHERE d.term_type IN ['SBD', 'BPCK', 'SCD', 'GPCK']
                  AND (toLower(d.name) CONTAINS toLower($drug_name)
                       ...
                       OR toLower(d.brand_name) CONTAINS toLower($drug_name)
                ...
                RETURN DISTINCT d.rxcui as rxcui, d.name as drug_name, d.full_name as full_name,
                       ...
                       CASE 
                         ...
                         WHEN toLower(d.brand_name) CONTAINS toLower($drug_name) THEN 0.8
...
search_brand_with_strength_query = """
                MATCH (d:Drug)
                WHERE d.term_type IN ['SBD', 'BPCK', 'SCD', 'GPCK']
                  AND (toLower(d.name) CONTAINS toLower($drug_name) OR toLower(d.sxdg_name) CONTAINS toLower($drug_name) OR toLower(d.brand_name) CONTAINS toLower($drug_name))
