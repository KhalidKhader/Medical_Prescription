...
def search_word_overlap_query(word_clause: str) -> str:
    return f"""
            MATCH (d:Drug)
            WHERE {word_clause}
            RETURN DISTINCT d.rxcui as rxcui, d.name as drug_name, d.full_name as full_name,
...
def word_conditions_query(word: str) -> str:
    return f"toLower(d.name) CONTAINS '{word}' OR toLower(d.full_name) CONTAINS '{word}'"