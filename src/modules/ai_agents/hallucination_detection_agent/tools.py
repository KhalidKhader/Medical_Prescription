...
def analyze_certainty_patterns(prescription_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Analyze certainty score patterns to detect potential hallucination indicators
    
    Args:
        prescription_data: Complete prescription data
        
    Returns:
        Dictionary with certainty analysis results
    """
    try:
        certainty_scores = []
        
        # Collect all certainty scores
        for section_name, section_data in prescription_data.items():
            if isinstance(section_data, dict) and "certainty" in section_data:
                if section_data["certainty"] is not None:
                    certainty_scores.append(section_data["certainty"])
            elif isinstance(section_data, list):
                for item in section_data:
                    if isinstance(item, dict) and "certainty" in item:
                        if item["certainty"] is not None:
                            certainty_scores.append(item["certainty"])
        
        if not certainty_scores:
            return {"analysis": "No certainty scores found", "suspicious": False}
        ...
        # All scores exactly the same (suspicious)
        if len(set(certainty_scores)) == 1 and len(certainty_scores) > 3:
            suspicious_indicators.append("All certainty scores identical")
        
        # All scores very high (potentially overconfident)
        if avg_certainty > 90 and min_certainty > 85:
            suspicious_indicators.append("Unusually high certainty across all fields")
        
        # Extreme variation without clear pattern
        if max_certainty - min_certainty > 80 and len(certainty_scores) > 2:
            suspicious_indicators.append("Extreme certainty variation")
        
        return {
            "analysis": f"Average certainty: {avg_certainty:.1f}, Range: {min_certainty}-{max_certainty}",
            "suspicious": len(suspicious_indicators) > 0,
            "indicators": suspicious_indicators,
            "score_count": len(certainty_scores)
        }
        
    except Exception as e:
        logger.error(f"Certainty pattern analysis failed: {e}")
        return {"analysis": "Analysis failed", "suspicious": True, "indicators": ["Analysis error"]}
